...
/* PJ103 relay module configuration console Giovanni Blu Mitolo 2020
   This is a basic example to show how PJ103 can be used practically.
   The Serial monitor is used to configure and print data from PJ103 */
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("PJ103 Relay module configuration console - Giovanni Blu Mitolo 2020");
  print_help();
};
...
void error_handler(uint8_t code, uint8_t data) {
  if(code == PJON_CONNECTION_LOST) {
    Serial.print("Connection lost - PJ103 ID ");
...
void print_help() {
  Serial.println();
  Serial.println("Use the Serial console input field to configure the PJ103 module.");
  Serial.println("Input commands guide: ");
  Serial.println("?!        -> Help and device info");
  Serial.println("H!        -> Relay on");
  Serial.println("L!        -> Relay off");
  ...
  if(Serial.available() > 0) {
    ...
    if(req_byte == '!') {
      ...
      if((char)packet[0] == 'C') {
        ...
        Serial.print("Setting PJ103 configuration: ");
      ...
      if((char)packet[0] == 'Q') {
        ...
        Serial.println("Flash the PJ103 module with PJ103.ino to unblock!");
        Serial.print(packet[1]);
        Serial.flush();
      }
      // Device id update
      if((char)packet[0] == 'I') {
        ...
        Serial.print("Setting PJ103 device id: ");
